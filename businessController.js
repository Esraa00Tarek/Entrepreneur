...
import { validateAttachmentContent } from '../Utilies/validation.js';
...
export const createBusiness = asyncHandler(async (req, res) => {
  ...
  if (req.attachments && req.attachments.length > 0) {
    for (const file of req.attachments) {
      const hasContactInfo = await validateAttachmentContent(file.url || file.originalName, file.fileType);
      if (hasContactInfo) {
        return res.status(400).json({ success: false, message: 'Your attachment contains contact information or forbidden content.' });
      }
    }
